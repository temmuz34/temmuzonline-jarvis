(function(root,factory){
  ...
  const initial=()=>({version:1,posts:[],snapshots:[],issues:[],checks:[],google:{},googleHistory:[]});
  ...
  const enums={platform:['Instagram','LinkedIn'],format:['Reels','Hikâye','Karusel','Görsel'],status:['draft','review','approved','published'],severity:['critical','high','normal'],issueStatus:['open','doing','done']};
  function valid(g){
    return !!g&&g.version===1&&Array.isArray(g.posts)&&g.posts.length<=300&&new Set(g.posts.map(p=>p?.id)).size===g.posts.length&&g.posts.every(p=>p&&str(p.id,200)&&day(p.date)&&/^([01]\d|2[0-3]):[0-5]\d$/.test(p.time)&&enums.platform.includes(p.platform)&&enums.format.includes(p.format)&&enums.status.includes(p.status)&&str(p.title,180)&&str(p.caption,4000)&&str(p.brief,4000)&&str(p.owner,100)&&str(p.asset,2000)&&(!p.asset||url(p.asset))&&(!['approved','published'].includes(p.status)||!!p.asset)&&(p.status!=='published'||url(p.publishedUrl))&&(!p.hashtags||(Array.isArray(p.hashtags)&&p.hashtags.length<=5&&p.hashtags.every(t=>str(t,60))))&&['brand','product','cta','imagePrompt','videoScript','voiceover','publishedUrl'].every(k=>p[k]===undefined||str(p[k],2000)))&&
      ...
      Array.isArray(g.checks)&&g.checks.length<=30&&g.checks.every(c=>c&&Number.isFinite(Date.parse(c.at))&&Array.isArray(c.results)&&c.results.length<=6&&c.results.every(r=>r&&url(r.url)&&Number.isInteger(r.status)&&r.status>=0&&r.status<=599&&num(r.ms)&&str(r.error,300)));
  }
  ...
  function analytics(g){
    if(g.google?.analytics){const a=g.google.analytics;return [`Google Analytics · ${a.dates.current.startDate} / ${a.dates.current.endDate}`,`${a.current.activeUsers??'—'} aktif kullanıcı · ${a.current.sessions??'—'} oturum · ${a.current.ecommercePurchases??'—'} satın alma · ${a.current.purchaseRevenue??'—'} TL gelir`,...Object.entries(a.changes).filter(([,d])=>d!==null).map(([k,d])=>`${k}: %${d.toFixed(1)} değişim`),`Kaynak: ${a.source} · ${a.at}`];}
  ...
  function siteReport(g){
    ...
    if(c?.pages)return [`Site sağlık puanı: ${c.score??'Ölçülemedi'} · Taranan URL: ${c.scanned}`,`Kritik: ${c.counts.critical} · Yüksek: ${c.counts.high} · Normal: ${c.counts.normal}`,...c.issues.slice(0,5).map(i=>`${i.title}\n${i.url}\nEtki: ${i.impact}\nDüzeltme: ${i.action}`),c.scope];
  ...
  function finding(expert,g){
    ...
    if(expert.id==='bora'||expert.id==='atlas'){const sc=g.google?.searchConsole;return {text:[...siteReport(g),...(sc?[`Search Console: ${sc.current.clicks} tıklama / ${sc.current.impressions} gösterim; ortalama konum ${sc.current.position}.`]:[])].join('\n'),source:'Site taraması / Search Console / kayıtlı bulgular'};}
    if(expert.id==='mira'){
      const images=g.checks.at(-1)?.issues?.filter(i=>i.code==='image-alt')||[];if(images.length)return {text:images.map(i=>`${i.url}: ${i.title}\n${i.action}`).join('\n'),source:'Gerçek HTML görsel alt metin denetimi; görsel kalite taranmadı'};
