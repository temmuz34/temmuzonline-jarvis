...
class ArtworkHumanoidEngine{
  ...
  pointer(e){if(this.reduced||!this.effects||this.assemblyActive||this.shockActive)return;const r=this.container.getBoundingClientRect();const px=e.clientX-r.left,py=e.clientY-r.top;const hx=this.mapX(320),hy=this.mapY(170),hw=this.rect.s*120,hh=this.rect.s*150;const inside=px>=hx-hw&&px<=hx+hw&&py>=hy-hh&&py<=hy+hh;if(!inside){this.targetYaw=0;this.targetPitch=0;return;}const nx=clamp((px-hx)/hw,-1,1),ny=clamp((py-hy)/hh,-1,1);this.targetYaw=nx*18;this.targetPitch=ny*-5;this.lastPointerAt=performance.now();}
  ...
  drawHeadTurn(){
    const c=this.ctx,maxAngle=18,ang=this.yaw*Math.PI/180,abs=Math.abs(this.yaw)/maxAngle;
    this.drawBodyOnly();
...
open.addEventListener('click',()=>inline.replay());exit.addEventListener('click',hide);$('main-humanoid-replay')?.addEventListener('click',()=>inline.replay());view.addEventListener('keydown',e=>{if(e.key==='Escape'){e.preventDefault();hide();}});
