'use strict';
const messages={401:'Google yetkilendirmesi gerekli.',403:'Bu Google hesabında gerekli erişim bulunamadı.',429:'Google API kullanım kotası geçici olarak doldu.'};
function createGoogleAuth(env=process.env,fetcher=fetch){
 ...
 async function access(){
  ...
  pending=(async()=>{
   ...
   if(!r.ok)throw Object.assign(new Error(messages[r.status]||messages[401]),{status:r.status});
 ...
 async function request(url,body,propertyLabel){
  try{
   ...
   if(!r.ok){if(r.status===401){token=null;expires=0;}console.error('Google API response',r.status,propertyLabel);throw Object.assign(new Error(messages[r.status]||`${propertyLabel} mülküne erişilemiyor.`),{status:r.status});}
   return await r.json();
