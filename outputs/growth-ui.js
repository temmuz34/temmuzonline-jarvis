...
window.createGrowthUI=function({read,change,refresh,toast,audit}){
  ...
  const status={draft:'Taslak',review:'İncelemede',approved:'Onaylı · yayınlanmadı'};
  ...
  function render(){
    ...
    $('site-check').innerHTML=check?`<h3>Son erişim kontrolü <small>${esc(new Date(check.at).toLocaleString('tr-TR',{timeZone:'Europe/Istanbul'}))}</small></h3>${check.results.map(r=>`<div><span>${esc(r.url)}</span><strong class="${r.status>=200&&r.status<400?'ok':'warning'}">${r.status||'Erişilemedi'}</strong><small>${r.ms} ms · ${esc(r.error||'HTTP yanıtı')}</small></div>`).join('')}`:'<p class="empty-state">Henüz erişim kontrolü yok. Canlı kontrol yerel rapor sunucusunda çalışır.</p>';
  ...
  $('plan-form').onsubmit=e=>{
    ...
    try{const generated=G.plan({start:f.start.value,days:Number(f.days.value),topic:f.topic.value.trim(),audience:f.audience.value.trim()});const ids=new Set(read().growth.posts.map(p=>p.id)),added=generated.filter(p=>!ids.has(p.id));
  ...
  function postDialog(id){
    ...
    const form=$('post-form');for(const k of ['title','date','time','owner','caption','brief','asset','status'])form.elements[k].value=p[k];bindClose();
    form.onsubmit=e=>{e.preventDefault();const current=read().growth.posts.find(p=>p.id===id);if(JSON.stringify(current)!==editingPost){$('post-error').textContent='Kayıt başka bir yerde değişmiş. Kapatıp yeniden aç.';return;}
      const updated={...current};for(const k of ['title','date','time','owner','caption','brief','asset','status'])updated[k]=form.elements[k].value.trim();
  ...
  $('audit-site').onclick=async()=>{const b=$('audit-site');b.disabled=true;try{await audit();toast('Erişim kontrolü tamamlandı. Sonuçlar kaydedildi.');}catch(e){toast(e.message);}finally{b.disabled=false;}};
