(function(root,factory){const api=factory(typeof module==='object'&&module.exports?require('./insight-engine.js'):root.InsightEngine);if(typeof module==='object'&&module.exports)module.exports=api;else root.ExecutiveBrief=api;})(typeof globalThis!=='undefined'?globalThis:this,function(engine){
 ...
 function make(state,report,now=Date.now()){
  if(report.snapshot){const s=report.snapshot;state={...state,growth:{google:s.google,checks:s.audit?[s.audit]:[],issues:s.issues,posts:s.contentCalendar}};}
  ...
  if(Number.isFinite(sales.revenue))kpi('Ciro',money(sales.revenue),example?'ÖRNEK SATIŞ VERİSİ':'Rapor satış kaydı',salesConfidence,sales.revenueChange??null);
  if(Number.isFinite(sales.orders))kpi('Sipariş',String(sales.orders),example?'ÖRNEK SATIŞ VERİSİ':'Rapor satış kaydı',salesConfidence);
  ...
  const priorityFindings=[...findings.filter(n=>n.severity==='critical'),...findings.filter(n=>n.severity==='high'&&n.origin!=='audit')];
  for(const n of priorityFindings)addAction('high',`${n.title} ${n.action}`,n.owner,n.source);
  for(const o of report.seoOpportunities||[]){addAction('normal',`${o.keyword}: ${o.recommendedAction}`,'Atlas','Search Console');opportunities.push(`${o.keyword}: ${o.reasons.join(', ')}`);}
  for(const n of findings.filter(n=>n.severity==='high'&&n.origin==='audit'))addAction('high',`${n.title} ${n.action}`,n.owner,n.source);
