(() => {
  ...
  function add(points, start, duration, color = '#10cfff', strength = .6, weight = .85) {
    curves.push({ points, flowing: points.map(p => point(p.x,p.y)), start, duration, color, strength, weight, seed: rand() * TAU });
  }
  ...
  for (const side of [-1, 1]) {
    ...
    // A single dotted boundary leaves visible black gaps along the silhouette.
    for (let j = 0; j < 1; j++) {
      const rim = skull.map(p => point(sx(p.x - j * .8), p.y));
      add(side < 0 ? rim.reverse() : rim, start + j * 18, 960, '#08eaff', .4, 1.0);
    }
    ...
    for (let j = 0; j < 1; j++) {
      ...
      add(ear, start + 240 + j * 24, 750, '#09cdff', .42, .8);
    }
    ...
    // Keep the neck and shoulder boundary as one row of separated particles.
    for (let i = 0; i < 1; i++) {
      let p = sample(point(402-i*.4,504),point(418-i,597),point(386-i,617),point(260-i*2,642+i));
      p = p.concat(sample(p[p.length-1],point(159,643+i),point(119,673+i),point(106,760+i)).slice(1));
      add(side < 0 ? p : mirror(p), side < 0 ? 4750 : 5900, 1160, '#08ddff', .4, 1.1);
    }
  }
  ...
  for(const side of [-1,1]) for(let i=0;i<30;i++){
    ...
    add(p,(side<0?5050:6020)+i*7,1070,i%9?'#0087e6':'#10ccff',.30,.7);
  }
  ...
  for (let i = 0; i < 42; i++) {
    ...
    add(left, reveal, 1070, '#16cfff', .52, .76);
    add(mirror(left), reveal+55, 1070, '#16cfff', .52, .76);
  }
  ...
  for (let i = 0; i < 16; i++) {
    ...
    add(p, 5900 + u*410, 1040, i%5 ? '#ff9c20' : '#ffcf6c', .28, i%5 ? .75 : 1.15);
  }
  ...
  function core(time,clock) {
    ...
    glow(0,0,mix(6,181,p),'#c75600',p*.36*pulse);
    glow(0,0,mix(3,132,p),'#ff8c06',p*.42*pulse);
    for(const s of coreDust){
      ...
      ctx.globalAlpha=p*pulse*(1-s.radius)*(.3+s.depth*.6);
  ...
  function entity(time,clock) {
    ...
    for(const c of curves){
      ...
      if(progress>0){
        ...
        path(flowing,progress,c.color,c.strength*1.65*pulse,c.weight*1.3);
        if(progress<1){
